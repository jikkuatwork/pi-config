...
/**
 * @param {string|undefined|null} fromUser CLI `--key` / `--imgbb-key` or `IMGBB_API_KEY`
 * @returns {string}
 */
export function resolveImgbbApiKey(fromUser) {
  const t = fromUser != null ? String(fromUser).trim() : "";
  if (t !== "") return t;
  throw new Error("ImgBB API key is required. Set IMGBB_API_KEY or pass --key/--imgbb-key.");
}
...
Environment:
  IMGBB_API_KEY   ImgBB key; required unless --key is provided

Options:
  --key <apiKey>        ImgBB key (or set IMGBB_API_KEY)
  --expiration <sec>    Auto-delete after 60–15552000 seconds (ImgBB)
  --json                Print full API data object instead of one URL per line

Examples:
  IMGBB_API_KEY=YOUR_KEY node scripts/imgbb-upload.mjs ./photo.png

...
const DEFAULT_AZURE_DEPLOYMENT = "gpt-image-1.5";
const DEFAULT_SIZE = "1536x1024"; // Closest native size for 16:9 downstream crops
const DEFAULT_QUALITY = "high";
const DEFAULT_OUTPUT_ROOT = path.join(os.homedir(), "Desktop");
const MAX_BATCH_IMAGES = 8;
const MAX_EDIT_REFERENCE_IMAGES = 5;
const DEFAULT_EDIT_TRANSPORT = "multipart";
const DEFAULT_TIMEOUT_MS = 300_000;
const DEFAULT_RETRIES = 1;
const DEFAULT_CLOMA_ROOT = path.join(os.homedir(), "Projects", "cloma");
...
async function handleConfig(args) {
  ...
  if (subcommand === "set") {
    ...
    if (!name) {
      exitWithError("Usage: config set <channel> --api-key ... [--base-url ...] [--model/deployment ...] [--provider openai|azure] [--api-version ...] [--edit-transport multipart|generations]");
    }
    config.channels ??= {};
    const existing = config.channels[name] || {};
    const provider = normalizeProvider(flags.provider ?? existing.provider);
    const editTransport =
    ...
    config.channels[name] = pruneEmpty({
      ...
      baseUrl: flags.baseUrl ?? flags.azureEndpoint ?? existing.baseUrl ?? DEFAULT_BASE_URL,
      model: flags.model ?? flags.deployment ?? existing.model ?? (provider === "azure" ? DEFAULT_AZURE_DEPLOYMENT : DEFAULT_MODEL),
      apiVersion: flags.apiVersion ?? existing.apiVersion ?? (provider === "azure" ? DEFAULT_AZURE_API_VERSION : undefined),
  ...
  if (subcommand === "auto-azure") {
    await handleConfigAutoAzure(maybeName, flags, config);
    return;
  }
  ...
  if (subcommand === "list") {
    ...
    for (const [name, channel] of channels) {
      ...
      const modelOrDeployment = channel.model || (provider === "azure" ? DEFAULT_AZURE_DEPLOYMENT : DEFAULT_MODEL);
      const base = channel.baseUrl || DEFAULT_BASE_URL;
      console.log(`${marker} ${name}  ${provider}  ${modelOrDeployment}  ${base}${transport}`);
    }
    return;
  }
  ...
  exitWithError("Usage: config <set|auto-azure|use|list|show> ...");
}

async function handleConfigAutoAzure(maybeName, flags, config) {
  const name = maybeName || "azure";
  config.channels ??= {};
  const existing = config.channels[name] || {};

  const hints = resolveClomaCredentialHints({
    clomaRoot: flags.clomaRoot,
    clomaEnv: flags.clomaEnv,
    clomaCredentials: flags.clomaCredentials,
  });

  const apiKey = flags.apiKey || existing.apiKey || envFirst("AZURE_OPENAI_API_KEY") || hints.apiKey;
  const baseUrl = flags.baseUrl || flags.azureEndpoint || existing.baseUrl || envFirst("AZURE_OPENAI_ENDPOINT") || hints.endpoint;
  const deployment = flags.deployment || flags.model || existing.model || envFirst("AZURE_OPENAI_DEPLOYMENT", "AZURE_OPENAI_MODEL") || hints.deployment || DEFAULT_AZURE_DEPLOYMENT;
  const apiVersion = flags.apiVersion || existing.apiVersion || envFirst("AZURE_OPENAI_API_VERSION") || hints.apiVersion || DEFAULT_AZURE_API_VERSION;

  if (!apiKey) {
    exitWithError("Could not find AZURE_OPENAI_API_KEY. Set it in env, pass --api-key, or add it to cloma env/credentials.");
  }
  if (!baseUrl) {
    exitWithError("Could not find AZURE_OPENAI_ENDPOINT. Pass --base-url (or --azure-endpoint), or define it in env/cloma.");
  }

  const editTransport =
    flags.editTransport !== undefined && flags.editTransport !== null && flags.editTransport !== ""
      ? parseEditTransport(flags.editTransport, "config auto-azure")
      : existing.editTransport;
  ...
  config.active = name;
  writeConfig(config);
  console.log(`Saved Azure channel "${name}" from cloma/env hints to ${configPath()}`);
}
...
async function handleGenerate(args) {
  ...
  const generationApi = apiPathFor(settings, "generations");
  const generationEndpoint = buildApiUrl(settings, generationApi);

  if (flags.dryRun) {
    console.log(JSON.stringify({ endpoint: generationEndpoint, payload, requestOptions }, null, 2));
    return;
  }
  if (flags.curl) {
    console.log(renderJsonCurl(settings, generationApi, payload));
    return;
  }

  const response = await fetchWithRetry(generationEndpoint, {
    method: "POST",
    headers: buildHeaders(settings, { "Content-Type": "application/json" }),
    body: JSON.stringify(payload),
  }, requestOptions);
  const json = await parseApiResponse(response);
  await saveImages(json, flags.output || defaultOutputPath("image", { prompt, templateId: flags.template }));
}
...
async function handleEdit(args) {
  ...
  const generationApi = apiPathFor(settings, "generations");
  const generationsEndpoint = buildApiUrl(settings, generationApi);
  const editApi = apiPathFor(settings, "edits");
  const editsEndpoint = buildApiUrl(settings, editApi);
  ...
  if (editTransport === "generations") {
    ...
    const requestOptions = resolveRequestOptions(flags);
    if (flags.dryRun) {
      console.log(JSON.stringify({ endpoint: generationsEndpoint, payload, requestOptions }, null, 2));
      return;
    }
    if (flags.curl) {
      console.log(renderJsonCurl(settings, generationApi, payload));
    ...
    const response = await fetchWithRetry(generationsEndpoint, {
      method: "POST",
      headers: buildHeaders(settings, { "Content-Type": "application/json" }),
      body: JSON.stringify(payload),
    }, requestOptions);
    const json = await parseApiResponse(response);
    await saveImages(json, flags.output || defaultOutputPath("edited", { prompt, templateId: flags.template }));
    return;
  }
  ...
  if (flags.curl) {
    console.log(renderMultipartCurl(settings, editApi, fields, files));
    return;
  }

  const { body, contentType } = buildMultipart(fields, files);
  const response = await fetchWithRetry(editsEndpoint, {
    method: "POST",
...
function resolveSettings(flags) {
  ...
  const provider = normalizeProvider(flags.provider ?? channel.provider ?? (flags.azure ? "azure" : undefined));

  const apiKey =
    flags.apiKey ||
    channel.apiKey ||
    (provider === "azure"
      ? envFirst("AZURE_OPENAI_API_KEY")
      : envFirst("GPT_IMAGE_API_KEY", "OPENAI_API_KEY", "API_KEY")) ||
    hints.apiKey;
  ...
  const baseUrl =
    ...
    channel.baseUrl ||
    (provider === "azure"
      ? envFirst("AZURE_OPENAI_ENDPOINT")
      : envFirst("GPT_IMAGE_BASE_URL", "OPENAI_BASE_URL", "BASE_URL")) ||
    hints.endpoint ||
    DEFAULT_BASE_URL;
  ...
  const model =
    ...
    channel.model ||
    (provider === "azure"
      ? envFirst("AZURE_OPENAI_DEPLOYMENT", "AZURE_OPENAI_MODEL")
      : envFirst("GPT_IMAGE_MODEL", "OPENAI_IMAGE_MODEL")) ||
    hints.deployment ||
    (provider === "azure" ? DEFAULT_AZURE_DEPLOYMENT : DEFAULT_MODEL);

  const apiVersion =
    flags.apiVersion ||
    channel.apiVersion ||
    (provider === "azure" ? envFirst("AZURE_OPENAI_API_VERSION") : undefined) ||
    hints.apiVersion ||
    (provider === "azure" ? DEFAULT_AZURE_API_VERSION : undefined);
  ...
  if (!apiKey && !flags.curl && !flags.dryRun) {
    exitWithError("Missing API key. Run config set <channel> --api-key ... or set OPENAI_API_KEY / AZURE_OPENAI_API_KEY.");
  }
  if (provider === "azure" && !baseUrl && !flags.curl && !flags.dryRun) {
    exitWithError("Missing Azure endpoint. Set AZURE_OPENAI_ENDPOINT or pass --base-url.");
  }

  return { provider, apiKey, baseUrl, model, apiVersion, organization, project, editTransport };
}
...
function isBooleanFlag(name) {
  return new Set(["json", "dryRun", "curl", "help", "version", "nativeOnly", "commonOnly", "embedLocalBase64", "azure"]).has(name);
}
...
function resolveClomaCredentialHints(options = {}) {
  const clomaRoot = path.resolve(String(options.clomaRoot || process.env.CLOMA_ROOT || DEFAULT_CLOMA_ROOT));
  const envPath = path.resolve(String(options.clomaEnv || path.join(clomaRoot, ".env")));
  const credsPath = path.resolve(String(options.clomaCredentials || path.join(os.homedir(), ".cloma", "credentials.json")));

  const envMap = readDotEnvFile(envPath);
  const creds = safeReadJson(credsPath) || {};
  ...
  return pruneEmpty({
    apiKey:
      ...
      openai.admin_key ||
      openai.api_key,
    endpoint: envMap.AZURE_OPENAI_ENDPOINT || envMap.AZURE_OPENAI_BASE_URL,
    deployment: envMap.AZURE_OPENAI_DEPLOYMENT || envMap.AZURE_OPENAI_MODEL,
    apiVersion: envMap.AZURE_OPENAI_API_VERSION,
  });
}
...
function normalizeProvider(value) {
  ...
  if (raw === "azure-openai") return "azure";
  if (raw !== "openai" && raw !== "azure") {
    exitWithError(`Unknown provider "${value}". Use --provider openai|azure.`);
  }
  return raw;
}

function apiPathFor(settings, route) {
  const provider = normalizeProvider(settings.provider);
  if (provider === "azure") {
    const deployment = encodeURIComponent(String(settings.model || DEFAULT_AZURE_DEPLOYMENT));
    if (route === "generations") {
      return `/openai/deployments/${deployment}/images/generations`;
    }
    if (route === "edits") {
      return `/openai/deployments/${deployment}/images/edits`;
    }
    exitWithError(`Unsupported Azure route: ${route}`);
  }
  if (route === "generations") return "/images/generations";
  if (route === "edits") return "/images/edits";
  exitWithError(`Unknown route: ${route}`);
}

function buildApiUrl(settings, apiPath) {
  const base = `${String(settings.baseUrl || "").replace(/\/+$/, "")}${apiPath}`;
  if (normalizeProvider(settings.provider) !== "azure") return base;
  const version = settings.apiVersion || DEFAULT_AZURE_API_VERSION;
  const glue = base.includes("?") ? "&" : "?";
  return `${base}${glue}api-version=${encodeURIComponent(version)}`;
}
...
function buildHeaders(settings, extra = {}) {
  ...
  const headers = {
    ...(provider === "azure"
      ? { "api-key": settings.apiKey || "$AZURE_OPENAI_API_KEY" }
...
function defaultOutputPath(prefix, context = {}) {
  const stamp = new Date().toISOString().replace(/[:.]/g, "-");
  const label = shortLabel(context.templateId || context.prompt || prefix);
  const folder = path.join(DEFAULT_OUTPUT_ROOT, `gi-${label}`);
  fs.mkdirSync(folder, { recursive: true });
  return path.join(folder, `${prefix}-${stamp}.png`);
}

function shortLabel(input) {
  const text = String(input || "image")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  const compact = text || "image";
  const parts = compact.split("-").filter(Boolean).slice(0, 4);
  const slug = parts.join("-") || "image";
  return slug.slice(0, 28).replace(/-+$/g, "") || "image";
}
...
function renderJsonCurl(settings, apiPath, payload) {
  ...
  const parts = ["curl -sS", "-X POST", shellQuote(buildApiUrl(settings, apiPath))];
...
Usage:
  gpt-image2 config set <channel> --api-key <key> [--base-url <url>] [--model/deployment <name>] [--provider openai|azure] [--api-version <yyyy-mm-dd>] [--edit-transport multipart|generations]
  gpt-image2 config auto-azure [channel] [--base-url <azure-endpoint>] [--deployment <name>] [--api-version <version>]
...
Common options:
  ...
  --provider <name>          Provider mode: openai (default) or azure
  --azure                    Shortcut for --provider azure
  --base-url <url>           Override API base URL / Azure endpoint, default ${DEFAULT_BASE_URL}
  --model <model>            Override model (OpenAI) or deployment (Azure)
  --deployment <name>        Alias of --model for Azure deployment naming
  --api-version <version>    Azure API version (default ${DEFAULT_AZURE_API_VERSION})
  --edit-transport <mode>    Edit API: multipart (default, POST /images/edits) or generations (JSON POST /images/generations with image[])
  --imgbb-key <key>          ImgBB key for generations edit (or set IMGBB_API_KEY)
  ...
  --size <size>              Example: 1024x1024, 1024x1536, 1536x1024 (default ${DEFAULT_SIZE}, 16:9-friendly)
  --quality <value>          API quality value (default ${DEFAULT_QUALITY})
...
Default output path (when --output is omitted):
  ~/Desktop/gi-<short-label>/<image|edited>-<timestamp>.png

...
此时 `gpt-image2 edit` 会向 `/images/generations` 发送 JSON：`http://` 或 `https://` 地址会原样传入数组；本地图片在提供 `IMGBB_API_KEY` / `--imgbb-key` 时会先上传到 ImgBB。只有明确需要 `data:image/...;base64,...` 时才使用 `--embed-local-base64`。单次最多 **5** 张参考图。**`--mask`** 仅适用于 multipart 的 `/images/edits`，在 `--edit-transport generations` 下不可用。

...
Then `gpt-image2 edit` sends JSON to `/images/generations`: `http://` or `https://` values are passed through unchanged, and local files are uploaded to ImgBB when `IMGBB_API_KEY` / `--imgbb-key` is provided. Use `--embed-local-base64` only when you intentionally want `data:image/...;base64,...` strings. At most **5** reference images per request. The **`--mask`** option is only for multipart `/images/edits`; it is not supported with `--edit-transport generations`.

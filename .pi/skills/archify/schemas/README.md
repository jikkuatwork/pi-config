...
`states`) accepts one optional `brand`. In this local edition, author only a
canonical string returned by `archify brands --json`. URL strings and legacy
digest-pinned `{ "url", "sha256" }` objects are rejected by the runtime privacy
gate even though the upstream-compatible generated shape validator recognizes
them. Brand resolution uses only the bundled vector catalogue and never opens a
network connection. Omitted `brand` preserves the prior output.
...
- `brandMark` — upstream-compatible shape; the local runtime narrows it to one bundled brand ID
...
Upstream compiled all five schemas with ajv's draft 2020-12 standalone
generator using `strict: true` and `allErrors: true`. This reviewed adaptation
vendors only the generated `renderers/shared/generated-validators.mjs`; it
omits the generator and development dependencies. Runtime validation therefore
has no npm or network dependency. `renderers/shared/validator.mjs` applies the
matching standalone validator before the renderer's own layout checks.

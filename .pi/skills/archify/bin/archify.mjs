...
async function commandDoctor() {
  ...
  const privacyCheckRuntime = path.join(skillRoot, 'scripts/check-privacy.mjs');
  const privacyCheckExists = fs.existsSync(privacyCheckRuntime);
  const privacyCheck = privacyCheckExists
    ? runNode([privacyCheckRuntime], { stdio: 'pipe' })
    : null;
  checks.push({
    label: 'No repo-data egress policy',
    ok: privacyCheckExists && privacyCheck.status === 0,
    missing: privacyCheckExists ? 0 : 1,
    invalid: privacyCheckExists && privacyCheck.status !== 0 ? 1 : 0,
    failureLabel: privacyCheckExists ? 'invalid' : 'missing',
  });
  ...
  const authoringReferences = [
    path.join(skillRoot, 'references', '03_authoring_contract.md'),
    path.join(skillRoot, 'references', '05_viewer_runtime.md'),
    path.join(skillRoot, 'references', '04_delivery_contract.md'),
  ];
...
async function commandBrands(args) {
  ...
  if (positional[0] === 'capture') {
    fail('Remote brand capture is disabled by the local no-egress policy. Use a bundled brand ID or omit brand.');
  }
  ...
  if (json) {
    console.log(JSON.stringify({
      ...
      count: marks.length,
      marks,
      fallback: 'Use a bundled brand ID or omit brand; remote brand capture is disabled.',
    }, null, 2));
    return;
  }
  if (!marks.length) {
    console.log(`No bundled brand matched "${query}". Omit brand or choose an ID from \`archify brands --json\`.`);
    return;
  }

...
Read this reference only after the Fast authoring path calls for more detail. The schemas and examples define the upstream-compatible shape; the mandatory local privacy gate in `01_privacy.md` further narrows brand values to bundled IDs.

## Schema lookup

Read both the mode schema and `schemas/common.schema.json`. The mode schemas use `$ref`, so the common file is where shared enums live. Never treat the legacy URL/object branch of `brandMark` as permission to use a network brand; this edition rejects it before rendering.
...
configuration before authoring. Record only evidence you actually verified in
the already-local checkout. Do not fetch, pull, clone, browse the origin, search
the web, or send source facts to another service. `--repo-root <path>` is
architecture-only and is accepted by architecture `render`, `validate`,
`deliver`, `preview`, and `compare`; workflow, sequence, dataflow, and lifecycle
reject it. Its evidence verifier uses local Git only. Never infer runtime
causality from file proximity or naming alone.

...
All viewer behavior remains subject to `01_privacy.md`. The page may load only
the fixed, no-referrer Google Fonts resources. It must not fetch repository
content, analytics, remote images, hosted data, or any other connection target.
Repository-evidence links are inert until a user deliberately follows them.
...
Viewer exports are local communication assets. They do not replace the checked
HTML, the deterministic delivery receipt, or a real visual review. Do not add a
hosted service, storage surface, dependency, schema branch, telemetry path, or
mobile product surface for these viewer-only capabilities. Never upload an
export as an implicit part of generation.

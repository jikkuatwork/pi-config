...
When text appears low-contrast, use the colors module to measure the rendered foreground/background pair and the accessibility module to classify the applicable requirement. Changing the project's colors remains a design decision unless the user asks for remediation.

## Font smoothing

Preserve the project's existing choice. These platform-specific properties can change perceived weight on macOS but are not a universal legibility improvement. If the actual typeface and target platforms justify them, apply the tested choice once at the root rather than inconsistently per component:

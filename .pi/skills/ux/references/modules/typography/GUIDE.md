...
The words themselves (button labels, error messages, empty states) are covered by the UX-writing module; semantic heading structure by the accessibility module; spatial RTL layout and logical CSS properties by the layout module; rendered-pair contrast measurement and color remediation by the colors module. This module owns how text renders, wraps, and behaves in mixed-direction content.

**Match the project's styling system.** Before suggesting or writing any fix, check how the codebase styles things and express every change in that system: Tailwind utilities in a Tailwind project, plain declarations in CSS, CSS Modules, styled-components or StyleX. The [cheat sheet](06_css_cheat_sheet.md) maps each declaration to its Tailwind equivalent. Never introduce a second styling approach just to apply a typography fix.

## Quick Reference

| Category | When to use | Reference |
| --- | --- | --- |
| Choosing fonts | Font categories, pairing, formats, typeface anatomy | [01_choosing_fonts.md](01_choosing_fonts.md) |
| Variable fonts & OpenType | Axes, weights, tabular numbers, stylistic sets | [02_variable_fonts_and_opentype.md](02_variable_fonts_and_opentype.md) |
| Spacing & sizing | Type scale, heading hierarchy, line-height, letter-spacing, text trimming | [03_spacing_and_sizing.md](03_spacing_and_sizing.md) |
| Wrapping & punctuation | Measure, wrapping, truncation, smart punctuation, RTL | [04_wrapping_and_punctuation.md](04_wrapping_and_punctuation.md) |
| Details & accessibility | Underlines, selection, forms, decorative text, contrast | [05_details_and_accessibility.md](05_details_and_accessibility.md) |
| CSS cheat sheet | Quick lookup of every property covered, with Tailwind equivalents | [06_css_cheat_sheet.md](06_css_cheat_sheet.md) |

## Core Principles

### 1. Serve the Right Format

Use `.woff2` (Brotli compression, broadly supported) on the web. `.woff` is a fallback only for very old browsers; `.ttf` and `.otf` are raw desktop formats with no web compression. How the files are loaded is the project's own concern; this module does not prescribe it.
...
Rarely use more than three fonts. Weight and size define hierarchy, but overusing them hurts readability quickly. Pair for contrast, not similarity. Treat `400`+ below roughly `18px` and very light weights only at display sizes as conservative starting points; verify the actual face, platform, contrast, and rendering rather than filing a finding from weight alone.
...
Start long-form body text near the browser default of `16px`, then judge it in the actual typeface, measure, platform, and product density. UI text can go smaller: `14px` is a useful starting point for inputs and menus (inputs still need `16px` on mobile, see principle 15), `13px` for captions, rarely below `12px`. When text appears low-contrast, use the colors module to measure the rendered pair and the accessibility module to classify the requirement; do not change colors unless asked.

### 17. Font Smoothing Is Optional

Preserve the project's existing rendering choice. `-webkit-font-smoothing: antialiased` and `-moz-osx-font-smoothing: grayscale` can alter perceived weight on macOS, but are non-standard/platform-specific and are not an automatic quality fix. Add them only for a deliberate, tested type treatment—once at the root rather than inconsistently per component.

### 18. Language and Bidi Behavior

Set `lang` so browsers and assistive technology choose the right pronunciation, quotes, and hyphenation. Set `dir` at the document or content boundary where direction changes, preserve digit order, and use `<bdi>` for isolated mixed-direction values when needed. Spatial mirroring and logical CSS properties belong to the layout module.
...
| Heading element picked for its default size | Choose semantics with the accessibility module, then set the visual size in CSS |
...
| Font smoothing varies by component without intent | Centralize the tested choice at the root, or remove the overrides |
...
Use this format only when the user asks for a standalone typography review. When the interface-review module orchestrates the review, provide domain evidence and findings to that module and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.

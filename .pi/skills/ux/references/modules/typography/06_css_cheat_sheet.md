...
One-line lookup for every typography CSS declaration covered by this module, with the Tailwind 4 equivalent. Where no utility exists, the arbitrary-value form is shown. Pick the column that matches the project: the declaration in plain CSS, CSS Modules, styled-components or StyleX codebases, the utility in Tailwind codebases.
...
| `-webkit-font-smoothing` + `-moz-osx-font-smoothing` | Optional platform-specific weight/rendering choice; test, then centralize at root | `antialiased` |

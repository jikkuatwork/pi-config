...
Use this as a stable baseline for comparison-harness chrome, not as a product design candidate. Keep it visually distinct and consistent across variants, but adapt syntax and any accessibility, safe-area, browser, or framework issue required by the target. Do not add a dependency for it.
...
- **Stable harness chrome.** Do not restyle it per variant or let product brand choices bias the comparison. Target-required accessibility and compatibility fixes are allowed.
...
Baseline wiring for a standalone trusted local HTML prototype. The `innerHTML` path is only for variant strings authored inside the prototype—never inject user or network content. In a framework, render components/state idiomatically, use a keyed remount for replay, and measure the highlight with refs plus the appropriate layout lifecycle.

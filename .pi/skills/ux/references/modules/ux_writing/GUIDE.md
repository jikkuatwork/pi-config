...
How copy renders (capitalization via `text-transform`, truncation, smart punctuation) is covered by the typography module; error markup and announcements (`aria-invalid`, live regions) by the accessibility module; room for translated strings by the layout module.
...
Use this format only when the user asks for a standalone writing review. When the interface-review module orchestrates the review, provide domain evidence and findings to that module and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.

...
Reviewed starting implementations for common cases. Adapt them to the target's existing tokens, primitives, browser matrix, and interaction frequency; do not paste a recipe when the project already has a compatible pattern.

Curves refer to the candidate `--ease-out`, `--ease-in-out`, and `--ease-drawer` values in `03_standards.md`. Define only tokens the target deliberately adopts. Every recipe also needs applicable focus/keyboard behavior, a static state cue, reduced-motion handling, and pointer/touch verification; omitted boilerplate here does not waive those requirements.
...
No bundle dependency and programmatically controllable. Compositor behavior and interruption semantics depend on the animated properties, browser, and how the returned `Animation` is managed; verify the target.

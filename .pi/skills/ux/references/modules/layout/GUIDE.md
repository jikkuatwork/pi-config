...
Hit-area sizes and focus behavior are covered by the accessibility module; visual polish (radius, shadows, animation) by the UI-polish module; line length and text spacing by the typography module.
...
| --- | --- |
| [Grouping & Alignment](01_grouping_and_alignment.md) | Space vs separators, alignment edges, logical properties, importance ordering |
| [Spacing & Adaptivity](02_spacing_and_adaptivity.md) | Spacing between targets, layout margins, progressive disclosure, full-bleed content, breakpoints, i18n growth |
...
Without an established density system, start with `12px` between adjacent bordered or filled controls and `24px` of clearance around borderless text- and icon-only controls. Compact layouts may use less when the accessibility module hit areas do not overlap and the controls remain visually distinct.
...
Use this format only when the user asks for a standalone layout review. When the interface-review module orchestrates the review, provide domain evidence and findings to that module and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.

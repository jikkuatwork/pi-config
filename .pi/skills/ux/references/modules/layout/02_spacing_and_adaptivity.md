...
WCAG target-size requirements, larger usability targets, and pseudo-element expansion are covered by the accessibility module; these clearances are in addition, so expanded hit areas never overlap.
...
- **Truncation cues.** Clamped text shows an ellipsis and a way to expand; see the typography module for truncation mechanics.

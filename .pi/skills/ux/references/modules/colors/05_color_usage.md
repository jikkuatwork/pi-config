...
How to deploy color in an interface: semantic tokens, meaning, emphasis, and appearance variants. For picking the values themselves, see [02_palette_generation.md](02_palette_generation.md); for checking pairs, see [03_accessibility_contrast.md](03_accessibility_contrast.md).
...
If a role has no token yet, add the token; don't borrow one that happens to have the right value today. In Tailwind projects, this is the `@theme` block; see [04_gamut_and_tailwind.md](04_gamut_and_tailwind.md).
...
Every custom color needs a light and a dark variant; dark mode derivation is covered in [02_palette_generation.md](02_palette_generation.md). Beyond that, users who enable increased contrast expect visibly stronger differentiation; supply it with `prefers-contrast`:
...
The increased-contrast variant should create a clearly stronger rendered distinction without destroying semantic hierarchy. Re-measure each pair using the target project's required contrast method; do not rely on one fixed OKLCH lightness delta as a pass condition.

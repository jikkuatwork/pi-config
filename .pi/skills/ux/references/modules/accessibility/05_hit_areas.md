...
| Standard | Minimum |
| --- | --- |
| WCAG 2.5.8 (AA) | 24×24 CSS px or an applicable specified exception |
...
- Use `touch-action: manipulation` only when the control's intended gestures and target browser behavior justify it; do not disable needed pan/zoom gestures.
- Preserve useful native tap feedback, or replace `-webkit-tap-highlight-color` only with another clear pressed-state cue.

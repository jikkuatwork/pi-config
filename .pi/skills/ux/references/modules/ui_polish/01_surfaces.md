...
- Start with an actually neutral low-alpha black in light mode and white in dark mode, or reuse a verified neutral image-edge token.
- Avoid accidental tinted neutrals that make the edge look dirty against the surrounding surface.
- Do not use the accent as a generic image outline; reserve accent outlines for meaningful focus/selection state.
- Verify over the image's real backgrounds and omit the treatment when an existing frame or sufficient edge contrast already solves the problem.
...
Pure neutral alpha values avoid accidental tint. If the design system has a deliberate neutral image-edge token, use and verify that token instead of adding a one-off value.

...
For ordinary monochrome UI glyphs, prefer one SVG drawn with `currentColor` and let CSS drive default/hover/selected/disabled color. Separate designed outline/fill variants are valid when shape—not only color—communicates selected state.
...
If the swap animates, follow the contextual icon guidance in [02_animations.md](02_animations.md) and the canonical motion module; reuse target tokens rather than fixed source values.
...
- Prefer SVG for scalable interface glyphs. Preserve raster artwork or platform assets when SVG is unavailable or semantically inappropriate, and provide density-appropriate sources.
...
Analyze composite icons part by part: a badge or slash overlay may keep its position even when the base glyph flips. Accessible names for icon-only buttons are covered by the accessibility module.

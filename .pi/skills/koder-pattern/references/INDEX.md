...
| Design/adopt/run a blind-orchestrator queue | `references/queues/mode-selection.md`, then `references/queues/blind-orchestration.md`, `references/queues/blind-briefs.md`, plus the normal queue and harnex routes |
...
- Delivery is the invariant: a queue does not imply blind mode, Harnex does not imply a worker chain, and planning/docs/metadata default to direct or supervised work. Before planning-only or blind authorization, disclose the product outcome, worker count, artifact count, wall budget, and stop gate. Load `references/queues/mode-selection.md` before choosing machinery.
...
- Blind orchestration is explicit opt-in after the delivery-cost gate. It is for substantial unattended/context-isolated work, not ordinary planning or metadata. Keep the universal protocol in this skill and the repo overlay small: active window, queue/stop gates, ownership, exact validation, assurance profile, and permissions. Use strict per-entry review only when entry risk justifies it; fail closed when declared isolation cannot be enforced.

...
| Adopt, run, or recover explicit blind orchestration | `references/queues/mode-selection.md`, then `references/queues/blind-orchestration.md` (includes briefs and recovery), plus model/run/harnex leaves |
| Harnex-backed queue entries | Queue route above plus `references/harnex/INDEX.md` |

## Queue principles

- Select the delivery shape before creating machinery (`mode-selection.md` is the canonical gate).
...
- Blind orchestration is explicit opt-in; never silently weaken a declared blind profile. Generic blind law stays in this skill; repo artifacts carry only the local overlay (`blind-orchestration.md`).

...
Use this reference for analytic water surfaces with shared displacement and normals, derivative-filtered detail, analytic reflection, heuristic refraction, absorption, and crest foam. For the bounded pool heightfield simulation and its volume optics, read the `interactive-pool-volume` example. Use [`spectral-ocean`](../spectral_ocean/GUIDE.md) for stochastic FFT seas.
...
- Route to [`spectral-ocean`](../spectral_ocean/GUIDE.md) when the target is implementation-level
